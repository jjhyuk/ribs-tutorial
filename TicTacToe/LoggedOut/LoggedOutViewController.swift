...
protocol LoggedOutPresentableListener: AnyObject {
  ...
  // 10) 유저가 로그인 버튼에 대한 액션을 전달해야 한다.
  func login(withPlayer1Name player1Name: String?,
             player2Name: String?)
}
...
, LoggedOutViewControllable {
  ...
  override func viewDidLoad() {
    ...
    buildLoginButton(withPlayer1Field: playerFields.player1Field,
  ...
  private func buildPlayerFields() -> (player1Field: UITextField,
  ...
  private func buildLoginButton(withPlayer1Field player1Field: UITextField,
                                player2Field: UITextField) {
  ...
  @objc private func didTapLoginButton() {
    listener?.login(withPlayer1Name: player1Field?.text,
